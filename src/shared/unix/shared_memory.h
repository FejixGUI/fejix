...
fj_err fj_unix_shared_realloc(
    struct fj_unix_shared_buffer *buffer, size_t size);

/** Closes the buffer's file descriptor, but does not unmap the memory.
    This is used when the shell still needs to use the buffer's memory, but the
    client handle is no longer needed and can be destroyed. */
