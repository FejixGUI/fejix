...
enum fj_error fj_unix_shared_alloc(struct fj_unix_shared_buffer *buffer, size_t size);
...
/** Closes the buffer'e file descriptor, but does not unmap the memory.
    This is used when the shell still needs to use the buffer'e memory, but the client handle
