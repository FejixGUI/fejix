/** Contains convenient memory allocation functions and macros.
    ...
    *x = 123;
    ...
    FJ_REALLOC_UNINIT(&x, 0, 10); // old size, new size
...
#define FJ_ALLOC_COPIED(OUT_OBJECT_PTR, SOURCE) \
    (fj_alloc_copied(                           \
        (void **) (OUT_OBJECT_PTR),             \
        (void *) (SOURCE),                      \
        sizeof(**(OUT_OBJECT_PTR))))

#define FJ_REALLOC_UNINIT(ARRAY_PTR, OLD_LENGTH, NEW_LENGTH) \
    (fj_realloc_uninit(                                      \
        (void **) (ARRAY_PTR), OLD_LENGTH, NEW_LENGTH, sizeof(**(ARRAY_PTR))))
...
#define FJ_FREE(OBJECT_PTR) \
    (fj_free((void **) (OBJECT_PTR), sizeof(**(OBJECT_PTR))))
...
/** Reallocates a block of memory for an array of items.
    ...
    If reallocation fails, this does not change the given pointer and does not
   free the old block.
    ...
    * `realloc(ptr, x, y, item_size) = realloc(y*item_size)`
    ...
    \returns Into `ptr`: a new pointer on success, does not change the pointer
    on failure. */
fj_err fj_realloc_uninit(
    void **ptr, size_t old_length, size_t new_length, size_t item_size);

/** Similar to `fj_realloc_uninit`, but all new items of the array are
