...
static fj_err_t client_core_init_window_class_name(struct fj_client * client)
{
    ...
    client->data.winapi->shell.window_class_name = window_class_name;
...
static void client_core_deinit_window_class(struct fj_client * client)
{
    ...
    if (client->data.winapi->shell.window_class_name == NULL) {
        return;
    }

    fj_free(client->data.winapi->shell.window_class_name);
}
