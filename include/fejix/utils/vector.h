...
#define FJ_DEFINE_VECTOR(VECTOR_TYPE_NAME, ITEM_TYPE)                                    \
    struct VECTOR_TYPE_NAME {                                                            \
        ...
        uint32_t length, capacity;                                                       \
                                                                                         ...
                                                                                         \
    static inline enum fj_status VECTOR_TYPE_NAME##_insert(                              \
    ...
    {                                                                                    \
        ...
        return FJ_STATUS_OK;                                                             \
                                                                                         ...
                                                                                         \
    static inline enum fj_status VECTOR_TYPE_NAME##_remove(                              \
        struct VECTOR_TYPE_NAME *vector, uint32_t index)                                 \
    {                                                                                    \
        return fj_vector_shrink(                                                         \
            (void **) &vector->items,                                                    \
            &vector->length,                                                             \
            &vector->capacity,                                                           \
            index,                                                                       \
            sizeof(*vector->items));                                                     \
                                                                                         ...
                                                                                         \
    static inline enum fj_status VECTOR_TYPE_NAME##_push(                                \
        struct VECTOR_TYPE_NAME *vector, ITEM_TYPE *item)                                \
    {                                                                                    \
        return VECTOR_TYPE_NAME##_insert(vector, vector->length, item);                  \
    }                                                                                    \
                                                                                         \
    static inline enum fj_status VECTOR_TYPE_NAME##_pop(struct VECTOR_TYPE_NAME *vector) \
    {                                                                                    \
        if (vector->length == 0) {                                                       \
...
/**
    ...
    Returns `FJ_STATUS_INVALID_USAGE` or `FJ_STATUS_OUT_OF_MEMORY`.
*/
FJ_PUBLIC
enum fj_status fj_vector_expand(
...
*/
FJ_PUBLIC
enum fj_status fj_vector_shrink(
    void **items, uint32_t *length, uint32_t *capacity, uint32_t index, size_t item_size);
